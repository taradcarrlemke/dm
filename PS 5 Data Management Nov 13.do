...
revrs grant_status
revrs support_dreamers
save YouGovKeyVariables, replace

//Collapse by state
tab state
collapse revimm_imp_issue imm_imp_issue grant_status id_and_deport revid_and_deport support_dreamers increase_borderpatrol revincrease_borderpatrol imm_status family_income religion_important hispanic party, by(state)
save YouGovStatekeyVariables, replace
...
generate prodreamers = (support_dreamers==1)
//Are the next 3 correct? 
generate GOP = (party== 1)
generate Dem = (party==2)
generate Indep = (party==3)
//Also need to create dummies for Hispanic and immigrant status. How about family income and religion?
save YouGovKeyVariablesDummies, replace

//Collapse by state
tab state
collapse prodeportation upborderpatrol prolegalization prodreamers revimm_imp_issue imm_imp_issue grant_status id_and_deport revid_and_deport support_dreamers increase_borderpatrol revincrease_borderpatrol imm_status family_income religion_important hispanic party, by(state)
save YouGovStateDummies, replace

//Look at YouGov data 
use YouGovKeyVariables, clear
tab revimm_imp_issue grant_status, mi //Interesting. Looks as though respondents who say that imm is "very high importance" lean anti-immigrant. "Somewhat high" leav pro. Less importance lean pro.
tab revimm_imp_issue support_dreamers, mi //This same as above.
tab revimm_imp_issue id_and_deport, mi //This reflects same as one above.
tab revimm_imp_issue increase_borderpatrol, mi //Reflects trend above.
tab revimm_imp_issue imm_status, mi //As length of time in US increases, "very high" importance increases. 
tab revimm_imp_issue religion_important, mi // The more religious believe immigration to be an important issue.
table revimm_imp_issue grant_status support_dreamers
table revimm_imp_issue id_and_deport increase_borderpatrol

/**************/
/**regressions*/
/**************/
//Run regressions on YouGov data
//I variable are various. D variable is "imm is important issue."
...
reg increase_borderpatrol imm_imp_issue, ro //Looks to be stat sig. R2 of 10.
...
use YouGovKeyVariables, clear
regress grant_status imm_imp_issue
outreg2 using grantstatus.doc, replace ctitle(model 1)

reg grant_status imm_imp_issue imm_status
outreg2 using grantstatus.doc, append ctitle(model 2)

reg grant_status imm_imp_issue imm_status hispanic
outreg2 using grantstatus.doc, append ctitle(model 3)

reg grant_status imm_imp_issue imm_status hispanic family_income
outreg2 using grantstatus.doc, append ctitle(model 4)

reg grant_status imm_imp_issue imm_status hispanic family_income party
outreg2 using grantstatus.doc, append ctitle(model 5)
...
use YouGovKeyVariables, clear
regress id_and_deport imm_imp_issue
outreg2 using id_and_deport.doc, replace ctitle(model 1)

reg id_and_deport imm_imp_issue imm_status
outreg2 using id_and_deport.doc, append ctitle(model 2)

reg id_and_deport imm_imp_issue imm_status hispanic
outreg2 using id_and_deport.doc, append ctitle(model 3)

reg id_and_deport imm_imp_issue imm_status hispanic family_income
outreg2 using id_and_deport.doc, append ctitle(model 4)

reg id_and_deport imm_imp_issue imm_status hispanic family_income party
outreg2 using id_and_deport.doc, append ctitle(model 5)

/***************/
/*visualization*/
/***************/ 
use YouGovKeyVariables, clear
graph bar (mean) imm_imp_issue, over(grant_status) ytitle(Grant Status)
graph hbar (mean) imm_imp_issue (mean) grant_status if state, over(state,label(labsize(tiny))) title(Immigration Importance: Should US Grant Status?)
//could do the graph above for other I variables
graph hbar (mean) id_and_deport (mean) imm_imp_issue if state, over(state,label(labsize(tiny))) title(Immigration Importance: US Should ID and Deport)
//Id and Deport is a 1=YES or 2=NO 
graph hbar (mean) grant_status (mean) imm_imp_issue if state, over(state,label(labsize(tiny))) title(Immigration Importance: Grant Immigration Status)
//Grant status to certain immigrants is a 1=YES or 2=NO
graph hbar (mean) support_dreamers (mean) imm_imp_issue if state, over(state,label(labsize(tiny))) title(Immigration Importance: Support Dreamers)
//Dreamers is a 1=YES or 2=NO
use YouGovStatekeyVariables, clear
...
merge 1:m state using YouGovStateKeyVariables
...
reg revimm_imp_issue ilrctotal //appears to be stat significant with a P=0.013 and a R2=15.
//is it correct that im_imp_issue needs to be reversed? 
//do I need to include robust? stat sig will change.
//not sure using imm status, hispanic, party and family income make sense as expressed here. Do I need to code as dummy variables? 
reg revimm_imp_issue ilrctotal imm_status hispanic party family_income 
...
graph hbar (mean) id_and_deport, over(state, label(labsize(tiny))) title(Importance of Identifying and Deporting Undocumented)
...
graph hbar (mean) imm_imp_issue, over(state, label(labsize(vsmall))) title(Immigration is an Important Issue)
