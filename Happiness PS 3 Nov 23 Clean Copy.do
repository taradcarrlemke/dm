...
keep region size xnorcsiz srcbelt educ income income06 partyid polview excldimm immjobs immameco immcrime happy govdook conleg confed affctlaw uscitzn voteelec vote08 vote12 polint1 relig race dem10fut dem10pst demtoday attrally amgovt
...
revrs confidence_in_govt, replace

//create dummy for race, political view and uscitz for regression purposes??? ask Dr Adam
...
use AttitudesImmigandGovt_GSSforHappinessClass, clear
tab region
collapse happy size norc_size_code beltcode demtoday dem10pst dem10fut political_view tougher less_crime notkeytorespect_USlaw take_jobs eco_benefit confidence_in_exec trust_in_govt Congress_cares political_interest attend_political_event, by(region) 
...
twoway (scatter Congress_cares ilrctotal, msize(small) msymbol(circle_hollow) mlabel(region) mlabsize(vsmall)), ytitle(Congress cares about constituent demands) xtitle(Sanctuary Status) title(Sanctuary Status and Congressional Attentiveness), lfit Congress_cares ilrctotal, jitter(1)ms(Oh) //currently including in paper
...
//something odd is going on with trust_in_govt variable mean.
