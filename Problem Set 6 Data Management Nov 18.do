...
//Create dummies for family income?? Don't think so because continuous.
use YouGovKeyVariables, clear
save YouGovKeyVariables, replace
...
reg ilrctotal imm_imp_issue, ro //appears to be stat insignificant with a P=0.094 and a R2=85.
...
reg ilrctotal imm_imp_issue family_income party imm_status

reg ilrctotal imm_imp_issue
outreg2 using id_and_deport.doc, replace ctitle(model 1)

reg ilrctotal imm_imp_issue imm_status
outreg2 using id_and_deport.doc, append ctitle(model 2)

reg ilrctotal imm_imp_issue imm_status family_income
outreg2 using id_and_deport.doc, append ctitle(model 3)

reg ilrctotal imm_imp_issue imm_status family_income party
outreg2 using id_and_deport.doc, append ctitle(model 4)
 

reg id_and_deport imm_imp_issue
outreg2 using id_and_deport.doc, replace ctitle(model 1)

reg id_and_deport imm_imp_issue imm_status
outreg2 using id_and_deport.doc, append ctitle(model 2)

reg id_and_deport imm_imp_issue imm_status family_income
outreg2 using id_and_deport.doc, append ctitle(model 3)

reg id_and_deport imm_imp_issue imm_status family_income party
outreg2 using id_and_deport.doc, append ctitle(model 4)
...
twoway (scatter imm_imp_issue ilrctotal), ytitle(Immigration is important issue) xtitle(Sanctuary Policy) title(Sanctuary Policy and Importance of Immigration), lfit imm_imp_issue ilrctotal, jitter(1)ms(Oh)
twoway (scatter imm_imp_issue ilrctotal, msize(small) msymbol(circle_hollow) mlabel(state) mlabsize(vsmall)), ytitle(Immigration is important issue) xtitle(Sanctuary Policy) title(Sanctuary Policy and Importance of Immigration), lfit imm_imp_issue ilrctotal, jitter(1)ms(Oh)
